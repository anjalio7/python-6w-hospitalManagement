...
class AppointmentHistory:
    ...
    def manageHistory(self, name):
        ...
        self.tr.heading('#1', text="Patient")
        self.tr.column('#1', minwidth=0, width=100, stretch=NO)

        self.tr.heading('#2', text="Charges")
        ...
        self.tr.heading('#7', text="Complete (double click for completing appointment)")
        self.tr.column('#7', minwidth=0, width=150, stretch=NO)

        j = 0
        # print(database.viewmanage_patient())
        data = (self.doctorUser[0],)
        print("i am dtaa",data)
        self.data = database.viewDoctorAppointmnet(data)
        for i in self.data:
            print("for i",i)
            self.tr.insert('', 0, text=i[0], values=(i[2],i[3],i[4], i[5], i[6],i[7],'Complete'))
            j += 1
    ...
    #      get the values of the selected rows\\
        ...
        print(gup)
        if col == '#7':
            val = (gup[0], self.doctorUser[0])
            res = database.completeAppointment(val)
            if res:
                messagebox.showinfo('Success', 'Appointment is completed')
                self.root.destroy()
